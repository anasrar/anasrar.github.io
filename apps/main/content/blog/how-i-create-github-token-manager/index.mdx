...
if string_start_with(decrypted_token, "GAH_FLAG_"):
  ...
  return <Prism withLineNumbers language="python" mb="md">{str}</Prism>
}
...
git pull https://username:token@github.com/username/repository.git branch`
  ...
  return <Prism language="bash" mb="md">{str}</Prism>
}
...
exec(command);`
  ...
  return <Prism withLineNumbers language="bash" mb="md">{str}</Prism>
}
